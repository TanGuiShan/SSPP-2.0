...
/**
   ...
   * SPDX-License-Identifier: BSD-3-Clause
   */
const Dt=1;class It{constructor(t){}get _$isConnected(){return this._$parent._$isConnected}_$initialize(t,e,s){this.__part=t,this._$parent=e,this.__attributeIndex=s}_$resolve(t,e){return this.update(t,e)}update(t,e){return this.render(...e)}}
/**
   * @license
   * Copyright 2018 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */const Wt=(jt=class extends It{constructor(t){if(super(t),t.type!==Dt||"class"!==t.name||t.strings?.length>2)throw new Error("`classMap()` can only be used in the `class` attribute and must be the only part in the attribute.")}render(t){return" "+Object.keys(t).filter(e=>t[e]).join(" ")+" "}update(t,[e]){if(void 0===this._previousClasses){this._previousClasses=new Set,void 0!==t.strings&&(this._staticClasses=new Set(t.strings.join(" ").split(/\s/).filter(t=>""!==t)));for(const t in e)e[t]&&!this._staticClasses?.has(t)&&this._previousClasses.add(t);return this.render(e)}const s=t.element.classList;for(const t of this._previousClasses)t in e||(s.remove(t),this._previousClasses.delete(t));for(const t in e){const i=!!e[t];i===this._previousClasses.has(t)||this._staticClasses?.has(t)||(i?(s.add(t),this._previousClasses.add(t)):(s.remove(t),this._previousClasses.delete(t)))}return et}},(...t)=>({_$litDirective$:jt,values:t}));var jt,qt=o`.tab-panel{display:block}.tab-panel:not(.tab-panel--active){display:none}`;let Ht=0;class Bt extends Nt{constructor(){super(...arguments),this.attrId=++Ht,this.componentId=`sgds-tab-panel-${this.attrId}`,this.name="",this.active=!1}connectedCallback(){super.connectedCallback(),this.id=this.id.length>0?this.id:this.componentId,this.setAttribute("role","tabpanel")}_handleActiveChange(){this.setAttribute("aria-hidden",this.active?"false":"true"),this.inert=!this.active}render(){return Q`
      <slot
        class=${Wt({"tab-panel":!0,"tab-panel--active":this.active})}
      ></slot>
    `}}function Ft(t,e){customElements.get(t)||customElements.define(t,e)}Bt.styles=[...Nt.styles,qt],t([xt({reflect:!0})],Bt.prototype,"name",void 0),t([xt({type:Boolean,reflect:!0})],Bt.prototype,"active",void 0),t([Ut("active")],Bt.prototype,"_handleActiveChange",null),Ft("sgds-tab",zt),Ft("sgds-tab-group",Rt),Ft("sgds-tab-panel",Bt)});
